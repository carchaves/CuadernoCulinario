...
/**
 * Inventario de utensilios y equipamiento. Misma mecánica que `Despensa` (páginas editables +
...
export function Menaje({ store, state }: { store: AppStore; state: AppState }) {
  ...
  const deletePage = (pageId: string) => {
    if (!confirm("¿Borrar esta sección y todo su menaje?")) return;
    mutate((s) => {
    ...
    <div className="menaje-root">
      ...
      <div className="cols">
        <section className="shelves">
          <h2 className="col-title">Secciones</h2>
        ...
        <section className="sheet">
          {!active ? (
            <div className="empty-card">
              <p className="empty-title">No hay secciones</p>
              <p className="empty-body">Creá una para empezar a cargar utensilios.</p>
            </div>
          ) : (
            <>
              ...
              <ul className="list">
                {active.ingredients.length === 0 && <li className="row-empty">Esta sección está vacía.</li>}
      ...
      {renaming && (
        <Modal title="Renombrar sección" onClose={() => setRenaming(null)}>
          <input
      ...
      {iconPicker && (
        <Modal title="Ícono de la sección" onClose={() => setIconPicker(null)}>
...
.menaje-root .mast-mark{font-size:28px;color:var(--olive);line-height:1}
.menaje-root .mast-title{font-family:'Fraunces',Georgia,serif;font-weight:600;font-size:31px;line-height:1;margin:0}
...
.menaje-root .search-results li + li{border-top:1px dotted var(--line)}
...
.menaje-root .shelf.on{border-color:var(--olive);box-shadow:inset 0 0 0 1px var(--olive)}
...
.menaje-root .shelf-name{font-family:'Fraunces',Georgia,serif;font-size:16px;font-weight:600}
.menaje-root .shelf-count{font-size:11.5px;color:var(--inkFaint)}
.menaje-root .shelf-actions{display:flex;gap:4px;border-top:1px dashed var(--rule);padding-top:7px}
...
.menaje-root .shelf-new{border-style:dashed;flex-direction:row;align-items:center;gap:6px}
...
.menaje-root .check{border:none;background:#E9EDD8;color:var(--olive);border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:14px}
...
.menaje-root .page-title{font-family:'Fraunces',Georgia,serif;font-weight:600;font-size:22px;margin:0;flex:1}
.menaje-root .page-count{font-size:12px;font-weight:600;color:var(--olive);background:var(--oliveSoft);border-radius:20px;padding:3px 10px}

.menaje-root .add-row{display:flex;gap:8px;padding:12px 0 4px;border-bottom:1px dotted var(--line)}
...
.menaje-root .row + .row{border-top:1px dotted var(--line)}
...
.menaje-root .badge-peso{background:var(--oliveSoft);color:var(--olive)}
.menaje-root .badge-unidad{background:var(--amberSoft);color:var(--amber)}
.menaje-root .row-name{font-size:15px;font-weight:500;overflow-wrap:anywhere}
.menaje-root .leader{flex:1 1 auto;min-width:14px;border-bottom:2px dotted var(--rule);transform:translateY(-3px)}
...
.menaje-root .qty-num{font-family:'JetBrains Mono',ui-monospace,monospace;font-weight:600;font-size:14.5px;color:var(--ink)}
...
.menaje-root .empty-title{font-family:'Fraunces',Georgia,serif;font-size:19px;font-weight:600;margin:0 0 6px;color:var(--ink)}
.menaje-root .empty-body{margin:0;font-size:13.5px}

.menaje-root .modal-input{width:100%;border:1px solid #E5E0D2;border-radius:9px;padding:9px 11px;font-family:inherit;
...
.menaje-root .icon-grid button{font-size:22px;background:#FBF9F3;border:1px solid #E5E0D2;border-radius:10px;padding:10px 0;cursor:pointer}
`;
